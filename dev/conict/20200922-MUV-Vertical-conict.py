...
#----diretorio---#
raiz='C:/Users/breno/Documents/Drive - ifsp.edu.br/Estudo/PIC/11 CONICT/repo/datafinal/'
...
def calc(sinicial, vinicial, diametro, massa, cd, termob, termoc):
    ...
    raio = diametro/2
    area = pi*(raio*raio)
    f = [((b*diametro*termob) + (termoc*0.5*p*cd*area*abs(vinicial))*vinicial)]
    time = [0]
...
def simulacao(h, diametrocm, massag, cd, arrasto, bola): #diametro em cm, massa em g, h em m
    global b
    vinicial = 0
    #---- Medidas da bola ----#
    ...
    diretorio=raiz+str(arrasto)+'/'+str(h)+'/'
    diretorio+=(bola+'/')
    ...
    if arrasto == 'p':
        ...
        #Matriz EM:
            ...
            #coluna 4 - Energia dissipada [0,1] (Em[i]/Em[t=0])       #adimensional
            
        Fr_p= zeros([len(a_p), 1])
        
        for i in range(len(v_p)):
            vquadrado=abs(v_p[i]*v_p[i])
            ...
            Em_p[i, 4] = float(Em_p[i, 2])/(massa*g*h)
...
tabela = array(tabela) #unidades tabela: cm, g, m/s, m/s, m, adimensional
...
for k in range(len(h_tabela)):
    for x in range(len(bolas_arr)):
        ...
        vterminal, v_indice = alturamin(massa_a, diametro_a, v_a, s_a, x)
        #altura = s_a[v_indice[0]]
        altura=v_indice
...
h = array([10, 50, 100]) #unidade: m
tabela_tempos=array([['Futebol', 0, 0, 0], ['Basquete', 0, 0, 0], ['Tênis', 0, 0, 0], ['Voleibol', 0, 0, 0], ['Tênis de mesa', 0, 0, 0]])
...
for j in range(len(h)):
    for i in range(len(bolas_arr)):
        ...
        diretorio=raiz+'p/'+str(h[j])+'/'+(bolas_arr[i, 0])+'/'
        ...
        f16.set_ylabel('Energia dissipada ($E_{M}/E_{PG}$)')
    ...
    dirp=raiz+'p/'+str(h[j])+'/'
    t_t, v_t, s_t, a_t = calcteorico(h[j], 0, 0, 0)
