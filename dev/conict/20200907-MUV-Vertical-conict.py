...
def simulacao(h, diametro, massa, coef, arrasto, bola): #diametro em cm, massa em g, h em m
    ...
    if arrasto == 'p':
        ...
        print('Arrasto invalido')
        return 0
    
    return t, v, s, a, Em, data_array
...
for j in range(len(h)):
    for i in range(len(bolas_arr)):
        ...
        f1.plot(t_v, v_v, bolas_arr[i])
        f2.plot(t_v, s_v, bolas_arr[i])
        f3.plot(t_v, Em_v[:, 2], bolas_arr[i])
        f4.plot(t_v, Em_v[:, 4], bolas_arr[i])
        ...
        f5.plot(t_p, v_p, bolas_arr[i])
        f6.plot(t_p, s_p, bolas_arr[i])
        f7.plot(t_p, Em_p[:, 2], bolas_arr[i])
        f8.plot(t_p, Em_p[:, 4], bolas_arr[i])
    ...
    f1.savefig(dirv+'vt_v.svg', format='svg', dpi=1200)
    f2.savefig(dirv+'yt_v.svg', format='svg', dpi=1200)
    f3.savefig(dirv+'em_t.svg', format='svg', dpi=1200)
    f4.savefig(dirv+'emd_t.svg', format='svg', dpi=1200)
    
    f5.savefig(dirp+'vt_p.svg', format='svg', dpi=1200)
    f6.savefig(dirp+'yt_p.svg', format='svg', dpi=1200)
    f7.savefig(dirp+'em_p.svg', format='svg', dpi=1200)
    f8.savefig(dirp+'emd_p.svg', format='svg', dpi=1200)
