...
tabela = array(tabela)
#unidades tabela: cm, g, m/s, m/s, m, adimensional
...
def alturamin(massa, diametro, vabs, s, i):
    ...
    vabs=vabs.tolist()
    n=list(map(lambda i: i>= round(vterminal,6), vabs)).index(True) 
    print(n)
    if n==0:
        s[0]=nan
...
for k in range(len(h_tabela)):
    for x in range(len(bolas_arr)):
        ...
        tabela[x, 3], tabela[x, 5] = alturamin(massa_a, diametro_a, v_a, s_a, x)
        #unidades passadas: kg, m, m/s, m, contador; retornadas: m/s, m  
        tabela[x, 4] = vmax_a #m/s
        tabela[x, 6], o = reynolds(vmax_a, diametro_a)

pd.DataFrame(tabela).to_csv(raiz+'tabela.csv', header=None, index=None)
...
for j in range(len(h)):
    for i in range(len(bolas_arr)):
        ...
        f13.plot(t_t, abs(v_t), colorcode+'--', linewidth='0.8', dashes=(2.5,10))
        f14.plot(t_t, s_t, colorcode+'--', linewidth='0.8', dashes=(2.5,10))
